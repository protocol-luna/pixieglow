import { MATRIX_HOMESERVER, MATRIX_TOKEN, MATRIX_USERNAME, BOT_SERVER, pickReplyStyle, config, watchConfig } from "../config.js";
...
import {
  isRecentBotActivity, markBotActivity, markReplied, getGlobalInactivityMs,
  trackSpeaker, canFollowUp, clearCooldown, setPaused, restoreState, startPruning,
} from "../state/state.js";
import { computeDelay, shouldIgnore, shouldReact, pickReaction } from "../behavior/mannerisms.js";
...
import { recordMessage, getFatigueMultiplier, getFatigueIgnoreBonus, pruneTopicFatigue } from "../state/topic-fatigue.js";
...
function clearTypingTimer(roomId: string): void {
  ...
  if (existing) { clearInterval(existing); typingTimers.delete(roomId); }
}

function drainSessionQueue(roomId: string, queued: { body: string; sender: string; reason: string; eventId: string }[]): void {
  if (queued.length === 0) return;
  const next = queued.shift()!;
  console.log(`[bot] session queue: resuming queued message in ${roomId.slice(0, 8)}`);
  void triggerLunaReply(next.body, next.sender, roomId, next.eventId, false, next.reason).then(() => {
    if (!sessionPaused.has(roomId)) drainSessionQueue(roomId, queued);
  });
}

const roomPending = new Map<string, { body: string; sender: string; reason: string; eventId: string }[]>();
function queuePending(roomId: string, body: string, sender: string, reason: string, eventId: string): void {
  const q = roomPending.get(roomId) ?? [];
  q.push({ body, sender, reason, eventId });
  roomPending.set(roomId, q);
}

function drainPendingForRoom(roomId: string): { body: string; sender: string; reason: string; eventId: string } | null {
  const q = roomPending.get(roomId);
  if (q && q.length > 0) {
    const next = q.shift()!;
...
// --- Session limit ---
function checkSessionLimit(roomId: string, callback: (roomId: string) => void): void {
  ...
  if (count >= config.sessionMessageLimit) {
    ...
    console.log(`[bot] session limit (${count}), pause ${config.sessionPauseSeconds}s`);
    setTimeout(() => {
...
function stripMention(text: string): string {
  return text.replace(new RegExp(`<${botId}>`, "g"), "").replace(new RegExp(botId.split(":")[0].replace("@", ""), "gi"), "").trim();
}
...
async function triggerLunaReply(
  ...
  const startTyping = () => {
    ...
    typingTimers.set(roomId, setInterval(() => { client.setTyping(roomId, 15000); }, 12000));
  };
  ...
  try {
    ...
    async function sendFragments(parts: string[], replyTo: string | undefined): Promise<string | null> {
      let accDelay = 0;
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (!frag) continue;
        if (i === 0) {
          const finalContent = hesitationWord ? `${hesitationWord} ${frag}` : frag;
          hesitationWord = "";
          firstPromise = client.sendText(roomId, finalContent, replyTo).then((id) => { isFirstChunk = false; markBotActivity(roomId); return id; }).catch(() => null);
        } else {
          ...
          hesitationWord = "";
          setTimeout(() => { client.sendText(roomId, fragContent).then(() => markBotActivity(roomId)).catch(() => {}); }, accDelay);
        }
      }
    ...
    function splitBurst(text: string): string[] {
      ...
      return [words.slice(0, split1).join(" "), words.slice(split1, split2).join(" "), words.slice(split2).join(" ")];
    }
    ...
    let hesitationWord = "";
    if (hasHesitation) hesitationWord = config.hesitationWords[Math.floor(Math.random() * config.hesitationWords.length)];

    startTyping();

    const fullText = await askLLM({ username: displayName, text: content, sessionId: roomId });
    ...
    if (chunks.length > 0 && Math.random() < config.typoChance) {
      ...
      if (result && text.includes(result.originalWord)) {
        ...
        console.log(`[bot] typo: "${result.originalWord}" → "${result.correctedWord}"`);
      }
    }

    const willEdit = typoResult && (config.typoCorrectionStyle === "edit" || (config.typoCorrectionStyle === "mixed" && Math.random() < 0.5));

    let firstMessageId: string | null = null;
    const parts = splitBurst(textToSend);
    firstMessageId = await sendFragments(parts, refStyle.messageReference ? eventId : undefined);
    ...
    if (typoResult && firstMessageId) {
      ...
      if (willEdit) {
        ...
        console.log(`[bot] typo corrected (sent clean): "${typoResult.correctedWord}" → "${typoResult.originalWord}"`);
      } else {
        await client.sendText(roomId, `${typoResult.originalWord}*`).catch(() => {});
    ...
    console.error(err);
    try { await client.sendReaction(roomId, eventId, "❌"); } catch { /* ignore */ }
  } finally {
    ...
    if (queued) {
      console.log(`[bot] room ${roomId.slice(0, 8)}: responding to queued message (${queued.reason})`);
      await triggerLunaReply(queued.body, queued.sender, roomId, queued.eventId, false, queued.reason);
    }
  }
}

async function handleCommand(body: string, roomId: string, result: TriggerResult): Promise<boolean> {
...
function handleSleep(result: TriggerResult, sleepBehavior: SleepBehavior, roomName: string): boolean {
...
async function handleRoomEvent(roomId: string, event: { event_id: string; type: string; sender: string; content: Record<string, unknown> }): Promise<void> {
  ...
  const result: TriggerResult = evaluateMessage(body, event.sender, roomId, roomName, botId, MATRIX_USERNAME, isDM);
  ...
  if (sessionPaused.has(roomId)) {
    queuePending(roomId, body, event.sender, result.reason ?? "mention", event.event_id);
    console.log(`[bot] ${roomName}: queued (session pause)`);
    return;
  }

  const lastMsg = sessionLastMessage.get(roomId);
  if (lastMsg && Date.now() - lastMsg > config.sessionResetMinutes * 60000) sessionCounts.delete(roomId);
  ...
  if (result.shouldRespond) {
    ...
    if (!isDM && (shouldIgnore(result.reason, sleepBehavior) || Math.random() < fatigueIgnoreBonus)) {
      console.log(`[bot] ${roomName}: ignored (${result.reason})${fatigueIgnoreBonus > 0 ? ` fatigue=${fatigueIgnoreBonus.toFixed(2)}` : ""}`);
      return;
    }
    ...
    const delay = computeDelay(result.reason, sleepBehavior, body.length, getGlobalInactivityMs());

    setTimeout(async () => {
      if (shouldReact(result.reason, sleepBehavior)) {
        const reaction = pickReaction();
        await client.sendReaction(roomId, event.event_id, reaction).catch(() => {});
      }
    }, delay);
    ...
    await triggerLunaReply(body, event.sender, roomId, event.event_id, isDM, result.reason);
    checkSessionLimit(roomId, (sid: string) => { void resetLLM(sid); });
    return;
  }
  ...
  if (canFollowUp(roomId, botId) && sleepBehavior !== "sleep") {
    ...
    const fatigueMul = getFatigueMultiplier(roomId);
    const delay = computeDelay("follow-up", sleepBehavior, body.length, getGlobalInactivityMs()) * fatigueMul;
    await new Promise((r) => setTimeout(r, delay));
    ...
    await triggerLunaReply(body, event.sender, roomId, event.event_id, isDM, "follow-up");
    checkSessionLimit(roomId, (sid: string) => { void resetLLM(sid); });
  }
...
async function processTimeline(roomId: string, timeline: { events: { event_id: string; type: string; sender: string; content: Record<string, unknown> }[] }): Promise<void> {
...
export async function startBot(): Promise<void> {
  ...
  if (initialSync.rooms?.join) {
    for (const roomId of Object.keys(initialSync.rooms.join)) {
      ...
      const roomName = initialSync.rooms.join[roomId].state?.events?.find((e) => e.type === "m.room.name")?.content.name as string ?? roomId;
