...
const REASONS: (keyof ConcentrationThresholds)[] = ["mention", "dm", "name", "keyword", "follow-up", "random"];

function getThresholds(reason: string | null): ConcentrationThresholds[keyof ConcentrationThresholds] {
  if (reason && REASONS.includes(reason as keyof ConcentrationThresholds)) return config.concentration[reason as keyof ConcentrationThresholds];
...
export function computeDelay(
  ...
  console.log(`[mannerisms] delay=${delay.toFixed(0)}ms (reason=${reason} sleep=${sleepBehavior ?? "none"} len=${msgLength ?? 0} idle=${inactivityMs ?? 0})`);
  return delay;
}

export function shouldIgnore(reason: string | null, sleepBehavior?: string | null): boolean {
  const t = getThresholds(reason);
...
export function shouldReact(reason: string | null = null, sleepBehavior?: string | null): boolean {
  const t = getThresholds(reason);
  let chance = t.reaction_chance;
  if (sleepBehavior === "slow" || sleepBehavior === "short") chance = Math.min(chance, 0.02);
  if (chance <= 0) return false;
  const roll = Math.random();
  const react = roll < chance;
  console.log(`[mannerisms] react=${react} (roll=${roll.toFixed(3)} < chance=${chance})`);
  return react;
}
