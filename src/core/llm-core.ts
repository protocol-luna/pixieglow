...
async function onlineRequest(item: QueueItem): Promise<void> {
  ...
  const text = await askOnline(item.userMessage, {
    onFirstToken: () => {
      if (!hasSentFirstToken) { hasSentFirstToken = true; currentItem?.onFirstToken?.(); }
    },
    onChunk: (chunk: string) => {
      emitWordTokens(chunk);
      currentItem?.onChunk?.(chunk);
...
function processQueue(): void {
  ...
  if (LLM_MODE === "direct") {
    void proxyRequest(item).catch((err) => {
      if (currentDoneHandler) { llmBus.off("done", currentDoneHandler); currentDoneHandler = null; }
      fail(err);
    });
...
export function askLLM(userMessage: UserMessage, callbacks?: { onFirstToken?: () => void; onChunk?: (chunk: string) => void }): Promise<string> {
  return new Promise((resolve, reject) => {
    requestQueue.push({ userMessage, resolve, reject, onFirstToken: callbacks?.onFirstToken, onChunk: callbacks?.onChunk });
    void processQueue();
  });
}
