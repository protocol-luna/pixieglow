...
export class MatrixClient {
  ...
  async whoami(): Promise<{ user_id: string; device_id: string }> {
    const res = await fetch(`${BASE}/_matrix/client/v3/account/whoami`, { headers });
  ...
  async sync(since?: string, timeout = 30000): Promise<SyncResponse> {
    ...
    const res = await fetch(`${BASE}/_matrix/client/v3/sync?${params}`, { headers });
    if (!res.ok) {
  ...
  async sendMessage(roomId: string, content: Record<string, unknown>): Promise<string> {
    ...
    const res = await fetch(`${BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/send/m.room.message/${txnId}`, {
      method: "PUT",
      headers,
  ...
  async sendText(roomId: string, text: string, replyTo?: string): Promise<string> {
  ...
  async sendEmote(roomId: string, text: string, replyTo?: string): Promise<string> {
    const content: Record<string, unknown> = {
      msgtype: "m.emote",
      body: text,
    };
  ...
  async sendReaction(roomId: string, eventId: string, emoji: string): Promise<void> {
    this.txnCounter++;
    const txnId = `pixie_${Date.now()}_${this.txnCounter}`;
    await fetch(`${BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/send/m.reaction/${txnId}`, {
      method: "PUT",
      headers,
      body: JSON.stringify({
        "m.relates_to": {
  ...
  async setTyping(roomId: string, timeout = 15000): Promise<void> {
    await fetch(`${BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/typing/${encodeURIComponent(this.userId)}`, {
      method: "PUT",
      headers,
  ...
  async markRead(roomId: string, eventId: string): Promise<void> {
    await fetch(`${BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/read_markers`, {
      method: "POST",
      headers,
      body: JSON.stringify({
  ...
  async getRoomMembers(roomId: string): Promise<MatrixMember[]> {
    const res = await fetch(`${BASE}/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/members`, { headers });
    if (!res.ok) return [];
  ...
  async joinRoom(roomIdOrAlias: string): Promise<void> {
    await fetch(`${BASE}/_matrix/client/v3/join/${encodeURIComponent(roomIdOrAlias)}`, {
      method: "POST",
      headers,
    }).catch(() => {});
  }
  ...
  async uploadMedia(data: Buffer | Uint8Array, mimeType: string, filename?: string): Promise<string> {
  ...
  async updatePresence(presence: "online" | "offline" | "unavailable"): Promise<void> {
    await fetch(`${BASE}/_matrix/client/v3/presence/${encodeURIComponent(this.userId)}/status`, {
      method: "PUT",
      headers,
