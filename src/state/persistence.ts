...
function defaultState(): PersistedState {
  return { paused: false, roomCooldowns: [], botActivity: [], lastSpeaker: [], responseCount: [] };
}
...
export function scheduleSave(state: PersistedState): void {
  ...
  saveTimer = setTimeout(() => {
    if (pendingState) persistState(pendingState).catch((err) => console.error("[persist] async write failed:", err));
    saveTimer = null;
  }, 500);
}
