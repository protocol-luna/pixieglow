...
import { isOnCooldown, isPaused, markReplied, setPaused } from "./state.js";
...
export function evaluateMessage(
	...
	const _botMention = `<${botId.toLowerCase()}>`; // fallback
	const isMentioned = hasWord(
		contentLower,
		botId.split(":")[0].replace(/^@/, ""),
	);
	...
	for (const name of config.names) {
		if (hasWord(contentLower, name.toLowerCase())) {
			log(
				roomName,
				`${sender}: "${body.slice(0, 60)}" → name (custom:${name})`,
			);
			markReplied(roomId);
			return { shouldRespond: true, reason: "name", botName };
		}
