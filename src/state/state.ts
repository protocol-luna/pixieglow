...
export function trackSpeaker(roomId: string, authorId: string): string | undefined {
...
export function canFollowUp(roomId: string, botId: string): boolean {
  ...
  if (ok) console.log(`[state] canFollowUp=true (room=${roomId.slice(0, 8)} followCount=${count})`);
  return ok;
}

export function isInConversation(roomId: string, botId: string): boolean {
  return isRecentBotActivity(roomId) && lastSpeaker.get(roomId)?.userId === botId;
}
...
export function startPruning(): void {
  setInterval(() => {
    ...
    for (const [key, ts] of roomCooldowns) { if (ts < cutoff) roomCooldowns.delete(key); }
    for (const [key, ts] of botActivity) { if (ts < cutoff) botActivity.delete(key); }
    for (const [key, entry] of lastSpeaker) { if (entry.timestamp < cutoff) lastSpeaker.delete(key); }
    for (const [key, count] of responseCount) { if (count <= 0) responseCount.delete(key); }
  }, PRUNE_INTERVAL);
}
